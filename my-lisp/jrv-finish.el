...
(defun split-window-maybe()
  ...
  (when (and jrv/settings-split-window-at-startup (> (window-total-width) 20))
    (ignore-errors
...
(defun jrv/finish-std-windows()
  ...
  (interactive)
  (jrv/finish-make-frame "Emacs Std")
  (let ((jrv/finish-org-buffer nil)
        (folder (jrv/mypaths-real-path "t")))
  ...
  (when (and jrv/settings-run-timelog
    ...
    (require 'jrv/end-of-day-functions)
    (defvar jrv/finish-timelog-timer
      (run-at-time "5:00pm" nil 'jrv/end-of-day-timelog)))
  )

(defun jrv/finish-mail-windows()
  "Set up mail windows in its frame"
  (interactive)
  (jrv/finish-make-frame "Emacs Notmuch")
  ...
  (condition-case err
    ...
    (error
     (let ((this-frame-name
       ...
       (if (or (string= this-frame-name "Emacs Std")
...
(defun jrv/finish-main (&optional no-std no-mail)
  ...
  (interactive)
  (message "jrv/finish-main %s %s" no-std no-mail)
  (unless no-mail
